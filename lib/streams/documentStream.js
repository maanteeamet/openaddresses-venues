...
function createDocumentStream(id_prefix, stats) {
    ...
    return through.obj(
        function write(record, enc, next) {
            ...
            /*  county              localadmin      unit                                locality
                maakond             omavalitsus     asula           street      number  place
                "Harju maakond"     "Rae vald"      "Rae küla"      Rae Tee     38
             */
            if (!isBlank(maakond) && !isBlank(omavalitsus) && !isBlank(asula) && !isBlank(street) && !isBlank(number)) {
                name = street + ' ' + number;
            } else if (!isBlank(maakond) && !isBlank(omavalitsus) && !isBlank(asula) && !isBlank(street)) {
                name = street;
            } else if (!isBlank(maakond) && !isBlank(omavalitsus) && !isBlank(asula) && !isBlank(place)) {
                name = place;
                locality = true;
            } else if (!isBlank(maakond) && !isBlank(omavalitsus) && !isBlank(asula)) {
                name = asula;
                if (contains(asula, 'linn') || contains(asula, 'linnaosa')) {
                    venue = true;
                }
            } else if (!isBlank(maakond) && !isBlank(omavalitsus)) {
                name = omavalitsus;
                venue = true;
            }
