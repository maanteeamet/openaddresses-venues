...
function hasAllProperties(record) {
    if (['VIITEPUNKT_X', 'VIITEPUNKT_Y'].every(function (prop) {
        return record[prop] && record[prop].length > 0;
    })) {
        return (
            (record.TASE6_NIMETUS_LIIGIGA && record.TASE6_NIMETUS_LIIGIGA.length > 0) ||
            (record.TASE7_NIMETUS_LIIGIGA && record.TASE7_NIMETUS_LIIGIGA.length > 0)
        ) || (
            (record.TASE2_NIMETUS_LIIGIGA && record.TASE2_NIMETUS_LIIGIGA.length > 0) ||
            (record.TASE3_NIMETUS_LIIGIGA && record.TASE3_NIMETUS_LIIGIGA.length > 0) ||
            (record.TASE4_NIMETUS_LIIGIGA && record.TASE4_NIMETUS_LIIGIGA.length > 0)
        );
    }
}
