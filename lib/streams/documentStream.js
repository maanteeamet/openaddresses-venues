...
function createDocumentStream(id_prefix, stats) {
    ...
    return through.obj(
        function write(record, enc, next) {
            ...
            let city = false;
            if (isBlank(name)) {
                if (!isBlank(omavalitsus)) {
                    name = omavalitsus;
                    city = true;
                } else if (!isBlank(vald)){
                    name = vald;
                    city = true;
                } else {
                    return;
            ...
            try {
                ...
                const addrDoc = new peliasModel.Document('openaddresses', (city ? 'venue' : 'address'), model_id)
                ...
                if (vald && vald.length > 0) {
                    addrDoc.addParent('locality', vald, addrId);
                }

                // console.log('beforePush - ', addrDoc);
                // if (city) {
                //     console.log('city: ', addrDoc);
                // }
                // if (oneHouseNrLog === 1) {
                //     console.log('address with number: ', addrDoc);
                // }
                if (isBlank((street + ' ' + number).trim())) {
                    console.log(addrDoc);
                }
                this.push(addrDoc);
            } catch (ex) {
